...
import cv2
import numpy as np
from ahm import buat_label_keaslian
import base64
...
def buat_label_keaslian_b64(output_name: str = "label_ali.png", dpi: int = 1200, simpan_b64: bool = True):
    ...
    # 1) Generate label (pakai param output_name, bukan output_path)
    buat_label_keaslian(output_name=output_name, dpi=dpi)

    # 2) Lokasi file output (asumsi ahm.py simpan ke folder data/)
    output_path = os.path.join("data", output_name)
